...
func CsharpRules(lang string,jsonFilePath string) []Rule {
	res := strings.Contains(jsonFilePath,",")
	var all []Rule
	if res{
		fileList := strings.Split(jsonFilePath,",")
		for z:=0;z<len(fileList);z++{
			jsonFile,err:=os.Open("/mnt/c/git/insider/rules/"+fileList[z])
			if err!=nil{
				log.Println("I am errror %s",err)
			}
			...
			log.Println("Rule File contain %d rules",len(ruleset.RuleSet))


			for i :=0; i< len(ruleset.RuleSet);i++{
				all =append(all,ruleset.RuleSet[i])
				log.Println(all[i])
			}
		}
	}else {
		jsonFile,err:=os.Open("/mnt/c/git/insider/rules/"+jsonFilePath)
		if err!=nil{
			log.Println("I am errror %s",err)
		}
		defer jsonFile.Close()
	...
	obj = Rule{}
	obj.ExactMatch = "<\\s*compilation\\s+debug=\"\\s*true\\s*\""
	obj.FileFilter = "web.config"
	obj.CWE = "CWE-12"
	obj.AverageCVSS = 2
	obj.Description_en = "The application is configured to display standard .NET errors. This can provide the attacker with useful information and should not be used in a production application. https://docs.microsoft.com/en-us/aspnet/web-forms/overview/older-versions-getting-started/deploying-web-site-projects/displaying-a-custom-error-page-cs"
	...
	//all = append(all, obj)
	log.Println("All rules are %s",all)
	return all
